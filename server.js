...
// official BNB mainnet registered vaults
...
// official impls from Flap docs (BNB mainnet)
const STANDARD_TOKEN_IMPL = process.env.STANDARD_TOKEN_IMPL || "0x8b4329947e34b6d56d71a3385cac122bade7d78d";
const TAX_TOKEN_V1_IMPL = process.env.TAX_TOKEN_V1_IMPL || "0x29e6383F0ce68507b5A72a53c2B118a118332aA8";
const TAX_TOKEN_V2_IMPL = process.env.TAX_TOKEN_V2_IMPL || "0xae562c6A05b798499507c6276C6Ed796027807BA";
...
function buildRegisteredVault(body) {
  if (body.caStore === "split") {
    ...
    for (const x of arr) {
      ...
      const r = ethers.getAddress(x.recipient);
      if (seen.has(r)) throw new Error("Split recipient duplicate");
      seen.add(r);
      const bps = Number(x.bps || 0);
      if (bps <= 0) throw new Error("Split bps invalid");
      total += bps;
      tuples.push({ recipient: r, bps });
    }
  ...
  if (body.caStore === "gift") {
    ...
    if (body.xHandle !== body.xHandle.toLowerCase()) throw new Error("Gift xHandle harus lowercase");
  ...
  if (body.caStore === "custom") {
    ...
    return { vaultFactory: ethers.getAddress(body.vaultFactory), vaultData: body.vaultData };
  }

  return null;
}

// CREATE2 vanity salt finder per official Flap docs
function minimalProxyBytecode(impl) {
  return "0x3d602d80600a3d3981f3363d3d373d3d3d363d73" + impl.toLowerCase().slice(2) + "5af43d82803e903d91602b57fd5bf3";
}
function pickTokenImpl({ taxRate, mktBps }) {
  if (Number(taxRate || 0) === 0) return STANDARD_TOKEN_IMPL;
  return Number(mktBps || 10000) === 10000 ? TAX_TOKEN_V1_IMPL : TAX_TOKEN_V2_IMPL;
}
function vanitySuffix({ taxRate }) {
  return Number(taxRate || 0) > 0 ? "7777" : "8888";
}
function predictCreate2Token(portal, impl, salt) {
  return ethers.getCreate2Address(portal, salt, ethers.keccak256(minimalProxyBytecode(impl)));
}
function findVanitySalt({ portal, taxRate, mktBps, maxIter = 250000 }) {
  const suffix = vanitySuffix({ taxRate });
  const impl = pickTokenImpl({ taxRate, mktBps });
  let salt = ethers.keccak256(ethers.randomBytes(32));
  for (let i = 0; i < maxIter; i++) {
    const predicted = predictCreate2Token(portal, impl, salt);
    if (predicted.toLowerCase().endsWith(suffix)) return { salt, predicted, impl, suffix, iterations: i + 1 };
    salt = ethers.keccak256(salt);
  }
  throw new Error("find vanity salt failed");
}
...
app.post("/api/processing", (req, res) => {
  try {
    ...
    if (body.caStore === "bnbshare") return res.json({ ok: true, label: "processing..." });
    const v = findVanitySalt({ portal: FLAP_PORTAL, taxRate: body.caStore === "none" ? 0 : Math.max(body.taxRate || 100, 100), mktBps: body.mktBps });
    res.json({ ok: true, label: "processing...", predicted: v.predicted, suffix: v.suffix, iterations: v.iterations });
  } catch (e) {
...
app.post("/api/launch", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    if (body.caStore === "bnbshare") {
      const feeAddr = body.feeRecipientAddress && ethers.isAddress(body.feeRecipientAddress) ? ethers.getAddress(body.feeRecipientAddress) : wallet.address;
      const payload = {
        name: body.name,
        symbol: body.symbol,
        metadataCid: meta,
        taxRate: Math.max(100, Math.min(1000, body.taxRate || 200)),
        quoteToken: "BNB",
        feeRecipients: [{ address: feeAddr, percent: 100 }],
      ...
      await tx.wait();
      const tokenAddress = d.predictedTokenAddress || d.tokenAddress || "";
    ...
    // official vanity salt
    const vanity = findVanitySalt({
      portal: FLAP_PORTAL,
      taxRate: body.caStore === "none" ? 0 : Math.max(body.taxRate || 100, 100),
      mktBps: body.mktBps
    });

    // registered/custom vault path
    if (body.caStore !== "none") {
      ...
      const params = {
        ...
        salt: vanity.salt,
        taxRate: Math.max(body.taxRate || 100, 100),
        migratorType: 1,
      ...
      const tokenAddress = receipt?.logs?.[0]?.address || vanity.predicted || "";
      addRecent(recentEntry(body, tx.hash, tokenAddress));
    ...
    const params = {
      ...
      quoteAmt,
      beneficiary: body.beneficiary && ethers.isAddress(body.beneficiary) ? ethers.getAddress(body.beneficiary) : wallet.address,
      permitData: "0x",
      extensionID: ethers.ZeroHash,
      extensionData: "0x",
      dexId: body.dexId,
...
app.listen(PORT, () => console.log(`BNBAgent registered vaults fixed on ${PORT}`));
